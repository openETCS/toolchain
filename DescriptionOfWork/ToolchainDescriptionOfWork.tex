...
The following activities  have already be identified and detailled in the following sections :
...
to code generation, some formal methods (for aexample the correct-by construction approaches) have shown their effectiveness in many industrial cases. Failing that ({\it i.e.} if not all the code is generated
...
A main part of the VnV activity is to check that all components of the system (software and hardware components) can be integrated together.

The first step  is to integrate the software components, depending of the choice of the modeling approach  formal proof or testing activities could be used to achieved the VnV.

The second step is to  integrate the software and hardware parts. Dynamic testing is currently  the more convenient approach  to  validate this integration.
...
Input: & WP1: Requirements for the open source ecosystm & continuously \\
