...
\begin{description}
\item[Scade.] This toolchain is based on Scade in connection with Papyrus, for which an integration already exisists.
...
Scade is the perfect candidate for a backup plan: While offering pretty much everything the project needs, it has a major fault: Scade is not open source.  At the same time, Scade is a backup that could be activated very quickly.  It is preferrable to model successfully with a closed-source solution than to fail with an open one.  But of course, modelling successfully with an open source solution is what we should strive for.
