...
process by first giving an overview of the process, then getting into
details for individual tool qualification, we then focus on the
incremental development of the tool chain and finally apply this
process to our tool chain implementation.
...
\tikzstyle{block} = [rectangle, draw, fill=blue!20, align=center, rounded corners]
	...
	\node [block, below of=n2,align=center] (l1) {Individual Tool Qualification (to be followed for each tool)};
		\node [block, below of=l1] (n10) {Toolchain Installation Qualification};
...
\subsection{Tool-to-Platform Integration Qualification}
This step ensures that the tool is correctly integrated within the
tool chain.
...
A new tool chain release deal with different type of changes :
\begin{enumerate} 
...
The first step is the feature Analysis.
...
The second step, the tool chain analysis consists of Bundle Analysis
and the work-flow Analysis.
The openETCS tool chain is integrated into the Eclipse platform. One
can analyses the sources of the tool chain to automatically derived
...
\item Dependencies
\end{itemize}
A list of tool should be produced and must be completed by the
qualification team. These information should be added directly in the
...
After the tool chain analysis phase, the individual tool qualification
starts. With the help of the tool list, the previous information given
(for former releases) and the basic use cases, the tool information
...
When all tool are completely defined, we may want to generate the
dependency list of the tools and the artifact matrix.
The former may be used to check if all tool can be correctly
integrated into the tool platform. 
The latter allow to check if all artifacts are correctly read and write
and/or all possible errors  are mitigated.
In case of re-qualification of the tool chain, the matrix can be used
to see which tool are impacted by the change.
